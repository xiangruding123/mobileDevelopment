...
Class.pt.auto = function(index){
    ...
    var titHeight = layero.find(doms[1]).height() || 0;
    var btnHeight = layero.find('.'+doms[6]).height() || 0;
...
Class.pt.tips = function(){
    var that = this, config = that.config, layero = that.layero;
    var layArea = [layero.width(), layero.height()], follow = $(config.follow);
    ...
    $(document).mousemove(function(M){
        if(conf.ismove){
            ...
            if(!config.moveOut){
                ...
                var setRig = win.width() - conf.move.width(), setTop = conf.setY;               
                offsetX < 0 && (offsetX = 0);
                offsetX > setRig && (offsetX = setRig); 
                offsetY < setTop && (offsetY = setTop);
                offsetY > win.height() - conf.move.height() + conf.setY && (offsetY = win.height() - conf.move.height() + conf.setY);
            }
...
layer.iframeAuto = function(index){
    if(!index) return;
    var heg = layer.getChildFrame('body', index).height();
