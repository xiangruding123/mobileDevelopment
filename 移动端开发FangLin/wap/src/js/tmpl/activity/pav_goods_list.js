!function(Global) {
    ...
    //设置微信分享信息
					var wx_title = GetQueryString("title");
					var wx_desc = GetQueryString("desc");
					var wx_link = location.href.split('#')[0];
					var wx_img = GetQueryString("img");
        ...
        $.ajax({
            ...
            success: function(data) {
                if (data&&data.error === "0") {
                    ...
                    if(type=="yes"){
                    	$(".brand_details_list li").remove();
                    }
                    $(".pullUp").before(html);
                     //计算图片大小
						var imgwidth =parseInt(($(".brand_details_list li").width()));
						$(".brand_details_list img").css({"width":imgwidth+"px","height":imgwidth+"px"});
